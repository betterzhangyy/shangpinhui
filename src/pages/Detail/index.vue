    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <Zoom :skuDefaultImg="skuInfo.skuDefaultImg" :skuImageList="skuInfo.skuImageList || []" />
          <!-- 小图列表 -->
          <!--放大镜效果 如果服务器没有及时返回数据，做兜底值处理，控制台不报错-->
...
<script>
  ...
  export default {
    ...
    methods: {
      //售卖属性高亮切换
      //第一个参数spuSaleAttrValue： 当前点击的属性值
      //第二个参数spuSaleAttrValueList：所有属性值
      changeActive(spuSaleAttrValue, spuSaleAttrValueList) {
        //排他操作（全部不选中，点击的那个再设置为选中）
        spuSaleAttrValueList.forEach(item => item.isChecked = 0)
        spuSaleAttrValue.isChecked = 1  //鼠标点击选中状态
      },
      changeNum(event) {
      ...
      async addOrUpdateCart() {
        try {
          ...
          //等待派发addOrUpdateCart这个action执行成功之后，再进行路由跳转
          this.$router.push({name: 'addcartsuccess', query: {skuNum: this.skuNum}})
          //如果是对象，不要用路由传参，地址栏不好看，可以使用本地存储（必须使用字符串）
