<template>
  <div class="clearfix selector">
    ...
    <div class="type-wrap" v-for="attrs in attrsList" :key="attrs.attrId">
      ...
      <div class="fl value">
        <ul class="type-list">
          <li v-for="(attrValue, index) in attrs.attrValueList" :key="index" @click="attrValueHandler(attrs, attrValue)">
            <a>{{attrValue}}</a>
          </li>
        </ul>
      </div>
  ...
  export default {
    ...
    methods: {
      ...
      trademarkHandler(trademark) {
        //给父组件Search组件传参---自定义事件
        this.$emit('getTrademarkInfo', trademark)
      },
      //平台售卖属性值的回调函数
      attrValueHandler(attrs, attrValue) {
        this.$emit('getAttrsInfo', attrs, attrValue)
      }
    },
  }
</script>
